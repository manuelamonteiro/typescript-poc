...
export async function insertHumor(humor: humorPost): Promise<QueryResult> {
...
export async function updateHumor(humor: humorUpdate, id: number | string): Promise<QueryResult> {
    return connectionDB.query(`UPDATE humor SET mood=$1, activities=$2 WHERE id=$3;`, [humor.mood, humor.activities, id]);
}

export async function removeHumor(id: number | string): Promise<QueryResult> {
